    ...
    Auth.auth().signIn(withEmail: emailField.text!, password: passwordField.text!) { (user, error) in
      if let _ = error{
        ...
        if (Auth.auth().currentUser?.isEmailVerified)!{
          guard let userId = Auth.auth().currentUser?.email?.replacingOccurrences(of: ".", with: "") else { return }
        ...
        if (error != nil){
          ...
          Auth.auth().signIn(with: credential) { (user, error) in
            if let _ = error{
              ...
              let userRef = self.ref.child((user?.email?.replacingOccurrences(of: ".", with: ""))!)
